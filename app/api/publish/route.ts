...
async function ensureUniqueSlug(baseSlug: string): Promise<string> {
  ...
  if (checkErr) {
    console.error("publish/checkErr:", checkErr);
    return baseSlug;
  }
  if (!existing || existing.length === 0) return baseSlug;

  const { data: siblings, error: likeErr } = await supabaseAdmin
    .from("articles")
    .select("slug")
    .like("slug", `${baseSlug}%`);

  if (likeErr) {
    console.error("publish/likeErr:", likeErr);
    return `${baseSlug}-2`;
  }
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: err?.message || "Publish crashed" }, { status: 500 });
  }
}
